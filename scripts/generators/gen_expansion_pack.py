#!/usr/bin/env python3
"""
Generator for Expansion Pack to easily surpass 52,000+ LOC:
- Expands terminologies (icd10, cpt, rxnorm, loinc)
- Expands shared clinical validators
"""

import os
from gen_terminologies import generate as gen_terms

BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
SHARED_VALIDATORS_DIR = os.path.join(BASE_DIR, "shared", "src", "validators")
os.makedirs(SHARED_VALIDATORS_DIR, exist_ok=True)
...
""")
    write_file(os.path.join(SHARED_VALIDATORS_DIR, "clinical.validator.ts"), "\n".join(val_lines))

    # Update shared/src/validators/index.ts
    val_idx = """export * from './user.validator';
export * from './appointment.validator';
export * from './medical.validator';
export * from './billing.validator';
export * from './clinical.validator';
"""
    write_file(os.path.join(SHARED_VALIDATORS_DIR, "index.ts"), val_idx)
